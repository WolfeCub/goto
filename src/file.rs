...
fn deserialize_goto_file() -> anyhow::Result<GotoFile> {
...
fn serialize_goto_file(goto_file: GotoFile) -> anyhow::Result<()> {
    ...
    serde_yaml::to_writer(io::BufWriter::new(f), &goto_file).with_context(|| "Failed to update file: `{path}`")?;
    Ok(())
}

pub fn get_project_for_current_dir() -> anyhow::Result<Project> {
    ...
    goto_file
        ...
        .with_context(|| "No projects found for current directory")
}
